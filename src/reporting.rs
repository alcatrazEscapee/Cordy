...
impl AsError for RuntimeError {
    fn format_error(self: &Self) -> String {
        match self {
            ...
            RuntimeError::TypeErrorArgMustBeChar(v) => format!("TypeError: Expected {} to be a single character string", v.format_error()),
...
impl AsError for ParserError {
    fn format_error(self: &Self) -> String {
        match &self.error {
            ...
            ParserErrorType::ExpectedCommaOrEndOfVector(e) => format!("Expected a ',' or ')' after vector literal, got {} instead", e.format_error()),
